	...
	public void repairModels(File[] brokenModels, int usersettings) {
		...
		for (int i = 0; i < brokenModels.length; i++) {
			...
			try {
				...
				Files.copy(knowledgeFile.toPath(), new File(iterationSpecificFolderName + "/" + knowledgeFile.getName()).toPath());
			} catch (IOException e) {
				e.printStackTrace();
			}
		}
		...
		try {
			File knowledgeFile = new File(Knowledge.KNOWLEDGE_FILE_NAME);
			Files.move(knowledgeFile.getAbsoluteFile().toPath(), new File(fixedModelFolderName + "/" + knowledgeFile.getName()).toPath());
		} catch (IOException e) {
			e.printStackTrace();
		}
	}
